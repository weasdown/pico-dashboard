...
_networks_file: str = '../.networks.json'
...
class FileNotFoundError(Exception):
    def __init__(self, file: str, message: str | None, *args: object) -> None:
        super().__init__(*args)
        self._file: str = file
        self._message: str = f' {message}' if message is not None else ''

    def __repr__(self) -> str:
        return f'FileNotFoundError: Could not find {self._file}{self._message}'
    ...
    def connect(self) -> None:
        ...
        while max_wait > 0:
            if self.status.Failed or self.status.Success:
                break
            max_wait -= 1
            print(f'\t- Waiting for connection to "{self.ssid}"...')
            time.sleep(1)
        ...
        if self._wlan.status() != WiFiStatus.GotIP:
            ...
            print(
                f'\nConnected to "{self.ssid}" (IP address: {self._wlan.ifconfig()[0]})\n')
...
def connect_to_wifi() -> None:
    ...
    try:
        ...
        if e.errno == ENOENT:
            message = f'\ncwd: {os.getcwd()}\nlistdir: {os.listdir()}'
            raise FileNotFoundError(_networks_file, message)
        else:
            raise
